...
def create_chatbot_graph(session: AsyncSession):
    ...
    workflow.add_node(
        "llm_decision",
        lambda state: llm_decision_node(state, tools)
    )
    workflow.add_node(
        "tool_execution",
        lambda state: tool_execution_node(state, tools)
    )
    ...
    workflow.add_edge("greeting", "llm_decision")
    ...
    workflow.add_edge("tool_execution", "state_update")
